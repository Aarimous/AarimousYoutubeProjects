...
func _onHighlightMoveableTiles(cell : Vector2):
	...
	_highlightTiles(cell, true, true)

			
func _highlightTiles(cell: Vector2, moveable : bool, capturable: bool):
	for neighborTile in _getAdjacentTile(cell):
		...
		if(capturable and _isValidCaptureTile(neighborTile)):
			neighborTile.highlightCapture()
			_highlightedCell.append(neighborTile.cell)

func _onCaptureTile(cell : Vector2):
	_unHighlightCell()
	_highlightTiles(cell, true, false)
	_captureCell = cell
		
func _isValidCaptureTile(tile) -> bool:
...
func _onMovePieceToNewCell(cell : Vector2):
	...
	if(_captureCell):
		...
		_captureCell = null
		_highlightTiles(cell, false, true)
