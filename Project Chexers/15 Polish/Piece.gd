...
func _setupMaterial(var color : Color):
	...
	$Cylinder/Super.set_surface_material(0,spatialMat)
...
func promoteToSuper():
	if(_isSuper == false):
		$PromotionAudioPlayer.play()
		self._isSuper = true
		$Cylinder/Super.visible = true
		
func getCaptured():
	if(_isPlayer1):
		Game.player1Pieces.remove(Game.player1Pieces.find(self))
	else:
		Game.player2Pieces.remove(Game.player2Pieces.find(self))
