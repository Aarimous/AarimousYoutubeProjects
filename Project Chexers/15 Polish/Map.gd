...
func _ready():
	...
	SignalBus.connect("StartNewGame", self, "_generateHexTileMap")
...
func _generateHexTileMap():
	...
	for cell in _getCellsToCreate():
		...
		id += 1
	SignalBus.emit_signal("UpdatePlayerScore")
	
func _addNewHexTile(cell : Vector2, id : int):
	var color =  Game.hex1Color if int(cell.y) % 2 == 0 else Game.hex2Color
...
func _addNewPieceToHex(cell : Vector2, isPlayer1 : bool):
	...
	if(isPlayer1):
		Game.player1Pieces.append(piece)
	else:
		Game.player2Pieces.append(piece)
...
func _highlightTiles(cell: Vector2, moveable : bool, capturable: bool, isPlayer1 : bool, isSuper: bool):
	for neighborTile in _getAdjacentTile(cell):
		...
		if(capturable and _isValidCaptureTile(neighborTile, isPlayer1, isSuper, cell)):
...
func _onMovePieceToNewCell(cell : Vector2):
	var piece = TilesDict[_selectedCell].getPiece()
	#TilesDict[cell].addPiece(piece)
	piece.moveToNewHexagonTile(TilesDict[cell])
	_unHighlightCell()
	_checkIfSuperPromotion(cell, piece)
	if(_captureCell):
		if(TilesDict[_captureCell].hasPiece()):
			TilesDict[_captureCell].getPiece().getCaptured()
		_captureCell = null
